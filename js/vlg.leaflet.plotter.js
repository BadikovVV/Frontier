L.Polyline.plotter = L.Polyline.extend({
    ...
    initialize: function (latlngs, options){
        this._lineMarkers=[]; // добавлено, Сахнов
        this._halfwayPointMarkers=[]; // добавлено, Сахнов
    ...
    onRemove: function(){
        ...
        L.Polyline.prototype.onRemove.call(this, this._map); // изменено, Сахнов
    },
    ...
    _bindMapClick: function(){
        //this._map.on('click', this._onMapClick, this); // изменено, Сахнов
    },
    ...
    _redraw: function(){
        ...
        this.redraw();
/*        var dist=0;
        for(pin=0;pin<(this._lineMarkers.length-1);pin++){
            dist+=this._lineMarkers[pin].getLatLng().distanceTo(this._lineMarkers[pin+1].getLatLng());
        }
        var polylineCenter=L.Polyline.prototype.getCenter.call(this);
        var popup = L.popup().setLatLng([polylineCenter.lat+0.0001,polylineCenter.lng])
            .setContent("L="+dist.toFixed(1)+" м").openOn(this._map);*/
    }
});
